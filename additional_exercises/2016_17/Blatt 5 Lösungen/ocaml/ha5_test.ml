...
let test_superfib () =
  for i = 0 to 10 do
    Assert.equal (X.superfib i) @@ Sol.superfib i
  done
...
let test_inv_cantor () =
  for i = 0 to 30 do
    ...
    Assert.is_true ((x', y') = (x, y) || (x', y') = (y, x))
  done
...
let test_scale_apply () =
  for i = 0 to 30 do
    let len = (Random.int 30) + 5 in
    let numbers = random_list len in
    let functions = [] in
    Assert.equal (X.scale_apply numbers functions) (Sol.scale_apply numbers functions)
  done
...
let test_singleton () =
  let p = { X.name = "Hugo"; X.age = 42 } in
  Assert.equal (X.singleton p) (X.Node (X.Leaf, X.Leaf, p))

let test_tree sorter =
  let persons = [{ X.name = "Hugo"; X.age = 42 };
                 { X.name = "Inge"; X.age = 1 };
                 { X.name = "Dagobert"; X.age = 33 };
                 { X.name = "Emil"; X.age = 42 };
                 { X.name = "Hanjo"; X.age = 42 };
                 { X.name = "Alfred"; X.age = 99 };
                 { X.name = "AAlbert"; X.age = 2 };
                 { X.name = "Lisa"; X.age = 11 };
                 { X.name = "Fibia"; X.age = 42 }] in
  let tree = List.fold_left (fun tree p -> X.insert p tree)
      (X.singleton { X.name = "Linus"; X.age = 12 }) persons in
...
let test_insert () =
  let rec to_sorted_list = function
      X.Node(lhs, rhs, p_node) ->
      (to_sorted_list lhs) @ [p_node] @ (to_sorted_list rhs)
    | X.Leaf -> [] in
  test_tree to_sorted_list

let test_to_sorted_list () =
  test_tree X.to_sorted_list

let () =
  add 10 @@ Test.make_simple_test ~title:"test_fib" test_superfib;
  add 20 @@ Test.make_simple_test ~title:"test_inv_cantor" test_inv_cantor;
  add 10 @@ Test.make_simple_test ~title:"test_scale_apply" test_scale_apply;
  ...
  add 10 @@ Test.make_simple_test ~title:"test_singleton" test_singleton;
  add 15 @@ Test.make_simple_test ~title:"test_insert" test_insert;
