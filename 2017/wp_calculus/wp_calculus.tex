...
3. Wir bemerken zunächst, dass l in jedem zweiten Schritt kleiner wird und wir daher für $l\ge0$ genau $2*l+2$ Schleifeniterationen benötigen, bis zum Verlassen der Schleife. Wir setzen daher initial $r=2*l$ und verringern r in jeder Schleifeniteration um eins. Wir zeichnen nun den Kontrollflussgraphen.
\begin{figure}[ht]
	\includegraphics[height=10cm]{exercise_3.png}
	\centering
\end{figure}
Für die Invariante bemerken wir (nachdem wir uns z.B. für $l=5$ für die ersten paar Schleifeniterationen tabellarisch alle Variablenwerte aufschreiben), dass gilt $(b\wedge r=2*l) \vee (\neg b\wedge r=2*l-1)$. Da nun r von l abhängt und wir später eine untere Schranke für r angeben müssen (da r streng monoton fällt), müssen wir auch die Größe von l in unsere Invariante aufnehmen (dabei darf der Fall $l<0$ nicht vergessen werden). Wir setzen also 
\begin{equation*}
	I\coloneqq \left(\left( b\wedge r=2*l\right) \vee \left(\neg b\wedge r=2*l-1\right)\right)\wedge l\ge -1 \vee l\le -1
\end{equation*}
und da wir nur zeigen sollen, dass das Programm terminiert, setzen wir weiters $J\coloneqq true$. Wir überprüfen nun die lokale konsistenz.
\begin{align*}
...
\end{align*}
Die letzte Implikation nehmen wir vor, da wir den Fall $l\le -1$ nie innerhalb unserer Schleife antreffen werden und wir ihn deshalb hier voraussehend verwerfen (er wird dann bei der Berechnung von $WP\llbracket l\ge 0\rrbracket(J,D)$ wieder hinzugefügt werden, allerdings würde er innerhalb der Schleife bei der Verifikation der unteren Grenze für r stören).\\
Insbesondere müssen wir nun überprüfen, ob $H\Rightarrow r>r-1$ gilt, um die Monotonitonie von r zu verifizieren. Mit $r>r-1\equiv0>-1\equiv true$ folgt die Implikation trivialerweise.
\begin{align*}
...
\end{align*}
Insbesondere gilt $D\Rightarrow r\ge-2$, somit haben wir eine untere Schranke für r.
\begin{align*}
WP\llbracket l\ge 0\rrbracket(J,D)&\equiv l\le -1 \vee l\ge 0 \wedge \left(\neg b \wedge r=2*l-1\wedge l\ge 0 \vee b \wedge r=2*l\wedge l\ge -1\right)\\
...
\begin{align*}
WP\llbracket r=2*i;\rrbracket(I)&\equiv l\le -1 \vee l\ge -1 \wedge\left(\left(\neg b \wedge 2*l=2*l-1\right) \vee \left(b \wedge 2*l=2*l \right)\right)\\
...
&\equiv l\le -1 \vee l\ge -1\eqqcolon B\\
WP\llbracket l=read();\rrbracket(B)&\equiv \forall l.\ l\le -1 \vee l\ge -1 \equiv \forall l.\ true\equiv true\eqqcolon A
\end{align*}
