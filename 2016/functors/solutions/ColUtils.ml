...
module ColUtils (C: Collection) = struct
        ...
        let tuple a b = if length a <> length b then raise Different_Length else 
                let get_outer c = let res = C.fold (fun (c',l) b -> if c'=c then (c'+1,Some b) 
                        else ((c'+1),l)) (0,None) b in match res with (_,Some l) -> l
                in let res = C.fold (fun (c,l) inner -> (c+1, C.add (inner, get_outer c) l)) (0, C.empty) a 
                        in match res with (_,l) -> l
        let every_nth a n = let n=n-1 in let res = C.fold (fun (c,l) a -> if c=n then (0,C.add a l) 
                else (c+1,l)) (0,C.empty) a in match res with (_,l) -> l
end
